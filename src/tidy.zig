...
const generated_paths = [_][]const u8{"src/profile/"};

const test_registries = [_][]const u8{
    "src/unit_tests.zig",
    "tools/profile_gen/unit_tests.zig",
};
...
const entry_points = [_][]const u8{
    "corpus_tests.zig",
    "fuzz_tests.zig",
    "generate_profile.zig",
    "main.zig",
    "root.zig",
    "round_trip_tests.zig",
    "unit_tests.zig",
};
...
fn tidy_lines(errors: *Errors, file: *const SourceFile) void {
    const generated = is_generated(file.path);
...
fn is_generated(path: []const u8) bool {
    for (generated_paths) |prefix| {
...
fn tidy_text(arena: Allocator, errors: *Errors, file: *const SourceFile) !void {
    ...
    if (is_generated(file.path)) return;
